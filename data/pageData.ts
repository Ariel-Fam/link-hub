export const pageData = [
  
  {
    id: "launchnarrative",
    name: "Launch Narrative Website",
    type: "Website",
    category: "Music Promotion",
    description: "This website promotes my futuristic Ai character and music album and serves as a narrative bridge between art, technology, music, and immersive web experiences.",
    status: "Active",
    src: "/launchNarrative.png",
    url: "https://www.launchnarrative.ca/"
  },
  {
    id: "astromechweb",
    name: "Astro Mech Web",
    type: "Website",
    category: "Descriptional Art",
    description: "This website is a place to explore my Ai Character in more detail, in terms of their origin, abilities etc.",
    status: "Active",
    src: "/astroWeb.png",
    url: "https://astro-mech-web.vercel.app/"
  },
  {
    id: "budget-track",
    name: "Budget Track",
    type: "Web App",
    category: "Finance",
    description: "A financial tracking application focused on budgeting, expense awareness, and long-term progress visualization.",
    status: "Active",
    src: "/budgetTrack.png",
    url: "https://budget-track-two.vercel.app/"
  },
  {
    id: "to-do-list-hub",
    name: "To-Do List Hub",
    type: "Web App",
    category: "Productivity",
    description: "A task management hub that helps users organize, prioritize, and track daily goals and responsibilities.",
    status: "Active",
    src: "/todoList.png",
    url: "https://www.todolisthub.com"
  },
  {
    id: "zodiac-view",
    name: "Zodiac View",
    type: "Web App",
    category: "Astrology",
    description: "An exploratory application that presents zodiac signs, traits, and astrological insights through a visual and interactive interface.",
    status: "Active",
    src: "/zodiacView.png",
    url: "https://zodiac-view.vercel.app/"
  },
  {
    id: "solar-system-view",
    name: "Solar System View",
    type: "Web App",
    category: "Astronomy",
    description: "An educational experience that allows users to explore planets, orbits, and relationships within the solar system.",
    status: "Active",
    src: "/solarSystem.png",
    url: "http://solar-system-view.vercel.app/"
  },
  {
    id: "multimodal-ai",
    name: "Multimodal AI Article",
    type: "Web Article",
    category: "Artificial Intelligence",
    description: "An educational article and visual experience explaining how modern AI systems integrate text, images, audio, and data into unified models.",
    status: "Active",
    src: "/multiModal.png",
    url: "https://www.multimodalarticle.com"
  }
]
